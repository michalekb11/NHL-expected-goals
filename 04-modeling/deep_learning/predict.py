...
df_preds['pred_G'] = np.array(predictions) 
df_preds['G'] = test_targets

######## Generate/save plot/distribution info ##########
########################################################
# Create a figure and axes
fig, axs = plt.subplots(nrows=3, ncols=1, figsize=(8, 10))
sns.histplot(data=df_preds, x="pred_G", bins=20, kde=True, ax=axs[0])
sns.histplot(data=df_preds[df_preds['player_id'] == 'mcdavco01'], x="pred_G", bins=20, kde=True, ax=axs[1])
sns.histplot(data=df_preds[df_preds['player_id'] == 'seidemo01'], x="pred_G", bins=20, kde=True, ax=axs[2])
plt.tight_layout()

# Distribution info
with open(f"{output_dir}/{model_name}_info.txt", 'w') as f:
    f.write(df_preds['pred_G'].describe().to_string())
